...
export class AccountPageComponent implements OnInit {
	...
	constructor(private m: DataModelManagerService, private route: ActivatedRoute, private datePipe: DatePipe) {
		this.currentDate =
			('0' + this.myDate.getDate()).toString().slice(-2) +
			'/' +
			('0' + (this.myDate.getMonth() + 1)).toString().slice(-2) +
			'/' +
			...
			if (Number(currentDateTemp[0]) > Number(subscriptionDateTemp[0])) {
				if (Number(currentDateTemp[1]) >= Number(subscriptionDateTemp[1])) {
					if (Number(currentDateTemp[2]) >= Number(subscriptionDateTemp[2])) {
						...
						this.m
							.usersUpdatePastDeliveries(this.m.user._id, this.m.user.pastDeliveries)
							.subscribe((u) => (this.message = u.message));

						let tempDate =
							subscriptionDateTemp[2] +
							'-' +
							subscriptionDateTemp[1] +
							'-' +
							subscriptionDateTemp[0] +
							'T00:00:00';
						...
						this.m
							.usersUpdateSubscriptionInfo(this.m.user._id, this.m.user.subscriptionInfo)
							.subscribe((u) => (this.message = u.message));
	...
	pause() {
		...
		this.m.usersGetByUsername(userName).subscribe((u) => {
			...
			if (this.lengthPause == 1) {
				...
				this.currentDateTemp =
					('0' + newDatePause.getDate()).toString().slice(-2) +
					'/' +
					('0' + (newDatePause.getMonth() + 1)).toString().slice(-2) +
					'/' +
					newDatePause.getFullYear().toString();

				this.m.user.subscriptionInfo[0].date = this.currentDateTemp;
				this.m
					.usersUpdateSubscriptionInfo(this.m.user._id, this.m.user.subscriptionInfo)
					.subscribe((u) => (this.message = u.message));
			} else if (this.lengthPause == 2) {
